...
class ForceStressOutputFromEdge(nn.Module):
    ...
    def forward(self, data: AtomGraphDataType) -> AtomGraphDataType:
        ...
        if fij is not None:
            ...
            s31 = rij[..., 2] * fij[..., 0]
            # cat last dimension
            _voigt = torch.cat([diag, s12.unsqueeze(-1), s23.unsqueeze(-1), s31.unsqueeze(-1)], dim=-1)
            ...
            if self._is_batch_data:
                ...
                sout = torch.zeros((nbatch, 6), dtype=_voigt.dtype, device=_voigt.device)
                _batch = _broadcast(batch, _s, 0)
                sout.scatter_reduce_(0, _batch, _s, reduce='sum')
            else:
                sout = torch.sum(_s, dim=0)

            data[self.key_stress] = torch.neg(sout) / data[self.key_cell_volume].unsqueeze(-1)
